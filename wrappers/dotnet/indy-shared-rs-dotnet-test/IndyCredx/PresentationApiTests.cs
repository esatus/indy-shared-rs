...
namespace indy_shared_rs_dotnet_test.IndyCredx
{
    public class PresentationApiTests
    {
        ...
        [Test, TestCase(TestName = "CreatePresentationAsync() returns a presentation object as JSON string for a proof with only self attest.")]
        ...
        public async Task VerifyPresentationJsonWorks()
        {
            ...
            List<string> revRegistries = new()
            {
                ...
                {
                    DefEntryIdx = 0,
                    Entry = (await RevocationApi.CreateRevocationRegistryFromJsonAsync(revRegJson)).Handle,
                    Timestamp = timestamp
                })
            };
